...
const fullData = reactive<s3DataType[]>([]);
...
<template>
  ...
  >
    <div class="flex justify-between pt-6 px-6 pb-0">
      <small
        class="text-lg font-normal text-[#313131] -tracking-[0.5px] block pb-0 relative z-20"
        >Latest Blocks</small
      >
      <div class="relative">
        <!-- <button
    ...
    <div class="block">
      <!--header-->
      <div
        class="py-0 px-[1.375rem] gap-y-0 gap-x-6 items-center mb-3 grid w-full mt-[1em] mx-0 grid-flow-col-dense grid-cols-4 md:grid-cols-[1fr,2fr,1fr,1fr]"
      >
        <h1
          class="font-light whitespace-nowrap text-[0.85rem] max-md:min-w-[85px] max-md:text-[0.7rem]"
          style="text-transform: uppercase"
        >
          Block
        </h1>
        <h1
          class="font-light whitespace-nowrap text-[0.85rem] max-md:min-w-[125px] max-md:text-[0.7rem]"
          style="text-transform: uppercase"
        >
          Block Hash
        </h1>
        <h1
          class="font-light whitespace-nowrap text-[0.85rem] max-md:min-w-[40px] max-md:text-[0.7rem]"
          style="text-transform: uppercase"
        >
          TXS
        </h1>
        <h1
          class="font-light whitespace-nowrap text-[0.85rem] last:w-full last:text-right max-md:min-w-[85px] max-md:text-[0.7rem]"
          style="text-transform: uppercase"
        >
          Time
        </h1>
      ...
      >
        ...
        >
          ...
          <div
            class="max-w-[120px] md:max-w-[150px] max-md:max-w-[170px] text-ellipsis overflow-hidden"
          >
            {{ item.content.blockHash }}
          </div>
          <div>{{ item.content.numberOfTransactions }}</div>
