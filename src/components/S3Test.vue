...
export default {
  data() {
    return {
      ...
      lastKeyFetched: "", // Store the key of the last object fetched
    };
  },
  methods: {
    async fetchS3Objects() {
      try {
        const listParams = {
          ...
          StartAfter: this.lastKeyFetched, // Only list objects after the last fetched key
        };
        const response = await s3Client.send(new ListObjectsV2Command(listParams));
        this.s3Objects = response.Contents || [];
        if (this.s3Objects.length > 0) {
          this.lastKeyFetched = this.s3Objects[this.s3Objects.length - 1].Key; // Update lastKeyFetched with the key of the last object
        }
      } catch (error) {
